...
class HandDetector(object):
    ...
    def __init__(self, dpt, fx, fy, importer=None, refineNet=None):
        ...
        # depth resize method
        self.resizeMethod = self.RESIZE_CV2_LINEAR

    def croppedNormDepth(self, size=(224,224)):
        com = self.calculateCoM(self.dpt)
        bounds = self.comToBounds(com, (200,200,200))
        cropped = self.getCrop(self.dpt, *bounds)
        cropped /= numpy.max(cropped)
        cropped_resize = cv2.resize(cropped, size)

        return cropped_resize, com
    ...
    def getCrop(self, dpt, xstart, xend, ystart, yend, zstart, zend, thresh_z=False):
        """
